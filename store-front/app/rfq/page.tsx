...
export default function RFQPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.category_id || !formData.title || !formData.description) {
      setError('Please fill in all required fields (Category, Title, Description)');
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Please login to post RFQ</h1>
          <Link href="/login?redirect=/rfq">
            <Button>Login</Button>
          </Link>
        </div>
      </div>
