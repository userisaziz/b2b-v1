...
import type { ProductApprovalRequest } from "@/services/approval.service";
...
export default function ProductApprovalPage() {
    ...
    // Fetch product approval requests
    useEffect(() => {
        const fetchRequests = async () => {
            try {
                ...
                const data = await getProductApprovalRequests(params);
                setRequests(data);
            } catch (err) {
    ...
    const filteredRequests = requests.filter(
        (request) =>
            request.productId.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const highValueItems = requests.filter(r => 
        r.productId.price > 1000 && r.status === "pending"
    ).length;
    ...
    const getRequestTypeBadge = (requestType: string) => {
        switch (requestType) {
            case "new":
                return <Badge variant="default" className="bg-blue-500/15 text-blue-700 hover:bg-blue-500/25 border-blue-200">New</Badge>;
            case "modification":
                return <Badge variant="secondary" className="bg-purple-500/15 text-purple-700 hover:bg-purple-500/25 border-purple-200">Modification</Badge>;
            default:
    ...
    const handleApprove = async (id: string) => {
        try {
            await approveProductApprovalRequest(id);
            ...
            alert("Failed to approve product request");
        }
    };
    ...
    const handleReject = async (id: string) => {
        ...
        try {
            await rejectProductApprovalRequest(id, rejectionReason);
            // Refresh the requests
            const params: any = {};
            if (statusFilter !== "all") params.status = statusFilter;
            if (requestTypeFilter !== "all") params.requestType = requestTypeFilter;
            ...
            alert("Failed to reject product request");
        }
    };

    const handleViewDetails = (id: string) => {
        navigate(`/approvals/products/${id}`);
        setDropdownOpen(null);
    };
    ...
    return (
        <div className="space-y-8 p-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-gray-900">Product Approval</h1>
                ...
                <CardHeader className="border-b bg-gray-50/50 px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Review product listings and approve or reject them
                            </CardDescription>
                        </div>
                                ...
                                <TableRow key={request._id} className="hover:bg-gray-50/50 transition-colors">
                                    <TableCell className="pl-6 font-medium">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span>{request.productId.name}</span>
                                        </div>
                                    </TableCell>
                                    <TableCell>{request.sellerId.name}</TableCell>
                                    <TableCell>{getRequestTypeBadge(request.requestType)}</TableCell>
                                    <TableCell className="font-medium">${request.productId.price.toFixed(2)}</TableCell>
